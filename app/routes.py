...
#     if current_user.is_authenticated:                                   # starting by making sure user is not logged in...
#         return redirect(url_for('index'))                               # ...if so it is redirected tp the index page
#
#     form = ResetPasswordRequestForm()
#     if form.validate_on_submit():                                       # when the form is submitted and valid...
#         user = User.query.filter_by(email=form.email.data).first()      # ...look up the user by the email provided in the form
#         if user:
#             send_password_reset_email(user)
#
#         flash('Check your email for the instructions to reset your password')
#         # the message above is displayed even if the email provided is unknown.
#         # This is so that clients cannot use this form to figure out if a given user is a memeber of not
#
#         return redirect(url_for('login'))
    #
...
#         return redirect(url_for('index'))                       # ...if not then go to the home page
#     user = User.verify_reset_password_token(token)              # determine who the user is by invoking the token verification method in the User class
#     if not user:                                                # the method returns the user if the token is valid, or None if not
#         return redirect(url_for('index'))                       # if the token is invalid redirect to the home page
#     form = ResetPasswordForm()                                  # if the token is valid present the user a second form, in which the new password is requested
#     if form.validate_on_submit():
#         user.set_password(form.password.data)                   # if the form is correctly submitted then change the password
#         db.session.commit()
